...
## Chat branch materialization hidden-message filter local closure (2026-08-14)

- DONE-LOCAL：`ConversationGraphService.branch_messages` 现在只返回 `visibility_state` 为 `NULL` 或 `visible` 的消息；隐藏的取消/失败占位不会进入 branch materialization 或 UI。新增回归测试验证可见祖先/后续保留、hidden 排除。
- 验证：PH4 `33 passed`；Ruff、`compileall`、`git diff --check` passed；本地 `/healthz` `200`；当前浏览器未认证。
- 本地切片不宣称 PH0–PH8 完成；生产服务器（`[production host]`）、备份、部署、回滚均 `NOT RUN/BLOCKED`。证据见 [`evidence/ekb-core-rebuild/local/2026-08-14-chat-hidden-filter.md`](./evidence/ekb-core-rebuild/local/2026-08-14-chat-hidden-filter.md)。不记录密码、token、API key 或私密地址。

## 2026-08-14 Chat parent-chain bridge local closure

- DONE-LOCAL：legacy `SqlStore.save_message` v4 bridge 对带 `turn_id` 的 `ASSISTANT` 优先选择同 `conversation`/`tenant`/`branch`/`turn` 的 `USER` 作为 parent，避免同秒 UUID 排序导致父子反向；无同 turn `USER` 时保留原 branch head 查询，无 v4 schema 时仍走 legacy 路径。
- 新增回归测试覆盖真实临时 v4 SQLite；验证为 PH4 `34 passed`、Ruff passed、`compileall` passed、`git diff --check` passed。
- 已认证本地浏览器可见真实 Assistant 会话、知识库和 root 分支，但现有旧会话仍可能是历史数据；不宣称旧数据已批量修复。本地切片不宣称 PH0–PH8 完成。
- 生产服务器（`[production host]`）、备份、部署、回滚仍 `NOT RUN/BLOCKED`。详细证据见 [`evidence/ekb-core-rebuild/local/2026-08-14-chat-parent-chain.md`](./evidence/ekb-core-rebuild/local/2026-08-14-chat-parent-chain.md)，不记录秘密。

## 2026-08-14 重启后真实浏览器 parent-chain 验证

- 本地 API 重启后 `/healthz` 为 `200`，使最新 `store.save_message` bridge 生效。
- 真实 Playwright 浏览器本地登录后新建 Assistant 会话并发送“重启后父链验证：请简短回答”；真实 SSE v2 返回远程 LLM + 当前授权 KB RAG 的回答。页面显示 branch selector=`root`，消息顺序为 `USER` → `ASSISTANT`。
- SQLite 核对同一 `turn` 的 `USER`/`ASSISTANT` 同 branch，且 `assistant.parent_message_id` 指向 `USER`。
