...
v3 的全栈扩展、数据流/sequence、migration ownership、安全和 rollout authority 为 [`v3 02_spec.md`](./pmos/features/2026-08-09_ekb-fullstack-v3/02_spec.md)，根索引为 [`docs/README.md`](./README.md)，竖切执行与 browser/contract gates 见 [`v3 03_plan.md`](./pmos/features/2026-08-09_ekb-fullstack-v3/03_plan.md) 和 [`v3 04_verification-matrix.md`](./pmos/features/2026-08-09_ekb-fullstack-v3/04_verification-matrix.md)。本文历史架构保持不变，只声明兼容：继续使用现有 FastAPI monolith、SQLAlchemy store、SQLite/PostgreSQL、`/api/v1`、AuthContext、audit、SSE v2 和 app-v2 adapters；本范围不引入 microservice/queue/worker。

v3 的新 UI 从零使用 design source 的 tokens/composition 和 app-v2 adapter facade，旧 layouts/components/CSS 不是实现或 rollback 目标。回滚通过 feature flag/API compatibility 到最后一个新 UI slice；生产 cutover 需先完成 import audit。v3 与 v2 frontend-only/禁止后端修改冲突的边界仅由 v3 successor 替代，其他历史架构记录仍有效。
