...
5. 开始 M1-01 至 M1-09 的逐任务实现和证据回填。
   ...
   - **M4-6 备份恢复演练已完成（2026-08-08）**：Spec NFR §10 要求"业务库、索引元数据、对象存储可恢复，M4 完成恢复演练并记录 RPO/RTO"，灾备手册 §6 已定备份策略框架。当前 SQLite 阶段（业务库+索引元数据合一，对象存储原文未落盘），落地 SQLite 适配版本。新增 `scripts/backup.py`：SQLite `VACUUM INTO` 在线一致性快照（不阻塞写入，生成紧凑无碎片副本）+ sha256 校验文件（`.sha256`）+ manifest 审计日志（`backup_manifest.jsonl`）+ 老备份自动清理（默认保留 7 份）+ `--dry-run` 预检（只校验源库可打开+表清单不写文件）+ 同秒多次备份序号防撞。新增 `scripts/restore.py`：恢复前 sha256 完整性校验（与 `.sha256` 文件比对，不一致抛异常）+ 恢复前自动备份当前库（防误恢复，命名 `pre_restore_*`）+ 文件级覆盖 + 恢复后行数校验（备份 vs 恢复后行数不一致抛异常）+ `--force` 跳过交互。新增 admin API `POST /api/v1/admin/backup`：用 `CAP_TENANT_PROVISION`（平台管理员能力）保护，触发备份 + 审计记录（成功记路径/大小/sha256/耗时/表数，失败记错误），复用 `scripts/backup.py` 的 `backup()` 函数。新增 `scripts/backup_restore_drill.sh`：端到端演练（种子 3 文档 3 chunk → 备份 → 删全部数据 → 恢复 → 校验行数一致 → 记录 RPO/RTO 到 JSON）。**演练结果**：RPO=0（VACUUM INTO 一致性快照，备份时刻即恢复点）；RTO=0.057s（SQLite 文件级恢复，停服务后文件覆盖）；备份耗时 0.069s；恢复后 documents=3/chunks=3 与备份前一致，校验通过。验证：pytest **99 passed**（+6 `test_backup.py`：备份快照一致性/dry-run/keep 清理/恢复数据完整性/恢复前自动备份/sha256 篡改检测）、ruff 全绿、端到端演练通过。`.gitignore` 已加 `backups/` 和 `drill_workspace/`。**生产扩展点**：切 PostgreSQL 后改用 `pg_dump` + WAL 归档（PITR）；对象存储用 S3 跨区域复制/版本管理；恢复顺序按灾备手册 §7.1（数据库→对象存储→索引/向量→校验→最小问答测试）。
